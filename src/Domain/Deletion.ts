...
export const deleteProductRelease = Effect.fn("Deletion.deleteProductRelease")(function* (
  ...
  CoreOutcome<{ deleted: true; activeVersion: string | null }, DeletionError>
> {
  ...
  return {
    statements: [
      ...
      db.update(product).set(productPatch).where(eq(product.id, productId)) as unknown as DbStatement,
      db
