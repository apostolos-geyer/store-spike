...
export const putVariant = Effect.fn("Catalog.putVariant")(function* (
  ...
  CoreOutcome<
    { variantId: string },
    "not_found" | "sku_taken" | "size_taken" | "invalid_stock"
  >
> {
  ...
  const statement = updating
    ? (db
        ...
        .set({
          ...
          ...(input.expectedShipAt !== undefined
            ? { expectedShipAt: input.expectedShipAt }
            : {}),
        })
...
export const adjustStock = Effect.fn("Catalog.adjustStock")(function* (
  ...
  CoreOutcome<{ stock: number }, "not_found" | "negative_stock">
> {
  const rows = yield* query(() =>
    db
